...
const bf=fs.readdirSync(path.join(root,'js/bank')); const files=['js/materi.js',...bf.filter(f=>!f.startsWith('bkn_')).map(f=>'js/bank/'+f),...bf.filter(f=>f.startsWith('bkn_')&&f!=='bkn_form.js').map(f=>'js/bank/'+f),'js/bank/bkn_form.js'];
...
for(const t of TOPICS){
  ...
  for(const q of qs){
    ...
    if(!Array.isArray(q.o)||q.o.length<4||q.o.length>5) problems.push(`${q.id} opsi bukan 4-5`);
    if(typeof q.a!=='number'||q.a<0||q.a>=q.o.length) problems.push(`${q.id} jawaban invalid`);
    const set=new Set(q.o.map(o=>o.trim())); if(set.size!==q.o.length && q.set!=='form') problems.push(`${q.id} opsi duplikat`); // peka huruf besar (soal kapitalisasi); set resmi 'form' dibiarkan apa adanya
    ...
    const lens=q.o.map(o=>o.length); const maxI=lens.indexOf(Math.max(...lens)); const avgOther=(lens.reduce((a,b)=>a+b,0)-lens[maxI])/(lens.length-1);
